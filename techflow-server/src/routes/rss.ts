  ...
  try {
    ...
    for (const feed of feeds) {
      ...
      try {
        ...
        storageService.updateFeedRefreshState(feed.id, { lastRefreshAt: startedAtIso, status: 'ok' });
        ...
        storageService.updateFeedRefreshState(feed.id, {
          lastRefreshAt: atIso,
          status: 'error',
...
router.get('/sync', async (req, res) => {
  try {
    ...
    const baseUrl = `${req.protocol}://${req.get('host')}`;
    const { latest, blocks } = storageService.getSyncBlocksForSource(url, Number.isFinite(since) ? since : 0, Number.isFinite(maxBlocks) ? maxBlocks : 20);
...
router.post('/clientSync', async (req, res) => {
  try {
    ...
    const savedUser = storageService.upsertUserFromClient({
      id: String(user.id),
    ...
    const savedFeeds = Array.isArray(feeds) ? storageService.upsertFeedsFromClient(savedUser.id, feeds) : [];
...
router.post('/refresh', async (req, res) => {
  try {
    ...
    const settings = storageService.getSettings();
    const feed = storageService.getFeeds().find(f => f.url === url);
...
router.post('/syncState', async (req, res) => {
  try {
    ...
    storageService.updateUserArticleStates(String(userId), states);
...
router.get('/syncState', async (req, res) => {
  try {
    ...
    const states = storageService.getUserArticleStates(userId, since);
