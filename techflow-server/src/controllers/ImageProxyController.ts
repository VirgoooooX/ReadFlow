    ...
    try {
      ...
      if (cacheFile && fs.existsSync(cacheFile)) {
        ...
        if (stats.size > 0) {
          ...
          if (requestedQ !== null) res.setHeader('X-TechFlow-Image-RequestedQ', String(requestedQ));
          res.setHeader('X-TechFlow-Image-Quality', String(quality));
          if (width && width > 0) res.setHeader('X-TechFlow-Image-Width', String(width));
          res.setHeader('X-TechFlow-Image-Mode', 'webp');
      ...
      if (raw) {
        ...
        if (requestedQ !== null) res.setHeader('X-TechFlow-Image-RequestedQ', String(requestedQ));
        res.setHeader('X-TechFlow-Image-Mode', 'raw');
        res.setHeader('X-TechFlow-Image-Source-ContentType', response.headers?.get?.('content-type') || '');
        res.setHeader('X-TechFlow-Image-Source-ContentLength', response.headers?.get?.('content-length') || '');
